...
class Entity:
    ...
    def do_full_svm(self):
        ...
        for i in range(self.iterations):
            ...
            svm_predictions = self.clf.predict(self.x_test_arr[i])
            self.precisions.append( metrics.precision_score(y_true=self.y_test_arr[i], y_pred=svm_predictions, pos_label=None) )
            self.recalls.append( metrics.recall_score(y_true=self.y_test_arr[i], y_pred=svm_predictions, pos_label=None) )
            self.f1s.append( metrics.f1_score(y_true=self.y_test_arr[i], y_pred=svm_predictions, pos_label=None) )
            self.accuracies.append(metrics.accuracy_score(self.y_test_arr[i], svm_predictions))
...
if __name__ == "__main__":
    ...
    for ent in entities:
        print ent.name + "\n---------------------------"
        print "Accuracy:  ", np.average(ent.accuracies)
        print "Recall:    ", np.average(ent.recalls)
        print "Precision: ", np.average(ent.precisions)
        print "F1-Score:  ", np.average(ent.f1s)
        print "==========================="
